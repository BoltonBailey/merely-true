...
Authors: Numina Fuse, Bolton Bailey
-/
...
This is a standalone copy of `ProblemFourteen` that does **not** depend on the
`sos` tactic (no `import SOS`), unlike my other version. I think it could be interesting to see how and if AI can optimize this without the SOS tactic, or if SOS is eventually added to mathlib, how it can be used to simplify the proof.
...
The layer certificate is built by hand from three univariate sign facts in `s`,
each proved by the `s = 1 + u` shift plus `nlinarith` (no `sos`):

* `c₄ = 3s⁴+6s²-1 > 0` — the parameter-free `w⁴`-coefficient of the layer-core
  quartic `G`;
* the degree-10 Schur complement `H₂ > 0`;
* the degree-18 Schur complement `H₃ ≥ 0`.

Viewing `G(s,·)` as a quartic in `w` with leading coefficient `c₄`, the
Positivstellensatz combination `c₄·G = (2c₄w² + …)² + H₂·(…)² + 4c₄·H₃ + …`
makes `G ≥ 0` an `nlinarith` step over these three facts. Feeding the resulting
`layer_core_6_4` to the general theorem yields `IsSupersolution (phiBar 6 4)`. -/

/-- `3s⁴+6s²-1 > 0` for `s ≥ 1`: the `w⁴`-coefficient of the layer-core quartic
`G`, independent of `K, c`. Proved by the `s = 1 + u` shift + `nlinarith`. -/
theorem hard_c4_pos {s : ℝ} (hs : 1 ≤ s) : 0 < 3*s^4 + 6*s^2 - 1 := by
  obtain ⟨u, hu, rfl⟩ : ∃ u, 0 ≤ u ∧ s = 1 + u := ⟨s - 1, by linarith, by ring⟩
  nlinarith [hu, pow_nonneg hu 2, pow_nonneg hu 3, pow_nonneg hu 4]
...
-- three `nlinarith` sign facts plus the heavy Positivstellensatz `nlinarith`
-- overrun the default budget
